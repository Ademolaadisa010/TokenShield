  ...
  await Promise.all(chunks.map(async chunk => {
    try {
      ...
      for (const pair of (data.pairs ?? [])) {
        ...
        if (!result[mint] || liq > result[mint].liquidity) {
          result[mint] = {
...
function computeRisk(liq: number, vol24h: number, mint: string): { risk: RiskLevel; score: number } {
  ...
  const risk: RiskLevel = score <= 10 ? "Safe" : score <= 25 ? "Low" : score <= 45 ? "Medium" : score <= 70 ? "High" : "Critical";
  return { risk, score };
}
...
const RC: Record<RiskLevel, { bg: string; text: string; border: string; bar: string; badge: string }> = {
  ...
  Medium:   { bg:"bg-amber-50",    text:"text-amber-700",   border:"border-amber-200",   bar:"bg-amber-500",    badge:"bg-amber-100 text-amber-800 border-amber-200"         },
  High:     { bg:"bg-orange-50",   text:"text-orange-700",  border:"border-orange-200",  bar:"bg-orange-500",   badge:"bg-orange-100 text-orange-800 border-orange-200"     },
  Critical: { bg:"bg-red-50",      text:"text-red-700",     border:"border-red-200",     bar:"bg-red-500",      badge:"bg-red-100 text-red-800 border-red-200"               },
  Unknown:  { bg:"bg-gray-50",     text:"text-gray-600",    border:"border-gray-200",    bar:"bg-gray-400",     badge:"bg-gray-100 text-gray-600 border-gray-200"             },
};
  ...
  return (
    <motion.tr ref={ref} initial={{ opacity:0, x:-12 }} animate={inView ? { opacity:1, x:0 } : {}}
      ...
      <td className="py-3.5 px-4">
        ...
        <p className="font-sans text-[10px] text-gray-400 tabular-nums">
      ...
      <td className="py-3.5 px-4">
        <div className="flex items-center gap-2">
          ...
          <span className={`font-sans text-xs font-bold px-2 py-0.5 rounded-full border ${rc.badge}`}>{asset.riskLevel}</span>
        </div>
      </td>
    ...
    try {
      ...
      if (solBalance > 0.001) {
        ...
        rawAssets.push({
          ...
          riskLevel:r.risk, riskScore:r.score, isVerified:true, dexUrl:solDex?.dexUrl,
        });
      }
      ...
      const withAlloc: Asset[] = rawAssets
        .map(a => ({ ...a, allocation: totalVal>0 ? (a.valueUsd/totalVal)*100 : 0 }))
        .sort((a,b) => b.valueUsd-a.valueUsd);

      setAssets(withAlloc);
    } catch (err) {
      console.error(err);
      setWallet(w => ({ ...w, error:"Failed to load wallet data. Check your connection and try again." }));
    } finally {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-[1360px] mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <AnimatePresence>
          {wallet.error && (
            <motion.div initial={{ opacity:0, y:-8 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0 }}
              className="mb-4 bg-red-50 border border-red-200 text-red-700 font-sans text-sm px-4 py-3 rounded-xl flex items-center justify-between gap-3">
              <span>⚠ {wallet.error}</span>
              <button onClick={() => setWallet(w => ({ ...w, error:null }))} className="text-red-400 hover:text-red-600 flex-shrink-0">✕</button>
            </motion.div>
          )}
        </AnimatePresence>
                    ...
                    return (
                      <>
                        ...
                        <div className="flex justify-between text-xs">
                          ...
                          <span className="font-sans font-bold text-orange-600">{risky.symbol} — {risky.riskLevel}</span>
                        </div>
                      </>
                    );
                  })()}
        ...
        <p className="font-sans text-xs text-gray-400 text-center mt-6">
          Prices from DexScreener. Risk scores are automated estimates — not financial advice. Always verify independently.
        </p>
      </div>
    </div>
  );
